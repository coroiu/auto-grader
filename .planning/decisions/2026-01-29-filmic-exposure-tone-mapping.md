...
**Problem Identified**: User feedback: "Snapseed protects highlights much more when lowering exposure. Your fix helped with shadows, but highlights still darken too much at -2 EV."

**Research Context**: After analyzing darktable's source code, discovered that:
1. Darktable uses pure linear exposure: `output = input × 2^exposure` (no midtone bias)
2. Tone mapping is a separate module (filmic, sigmoid)
3. Our context differs: We're processing **already tone-mapped sRGB TIFFs**, not RAW sensor data
4. Post-processing tools (Snapseed, Lightroom on JPGs) commonly use perceptual adjustments

**Decision**: Implement **Gaussian midtone-biased exposure** matching Lightroom's "exposure slider is really a midtones slider" behavior.
...
// Gaussian weight function centered on midtones
...
// Apply weighted exposure
linear *= pow(2.0, exposure * weight);
```

**Weight function behavior**:
- At `lum = 0.5` (midtones): `weight = 1.0` (full exposure effect)
- At `lum = 0.2` (shadows): `weight ≈ 0.75` (75% effect)
- At `lum = 0.8` (highlights): `weight ≈ 0.75` (75% effect)
- At `lum = 0.0` (black): `weight = 0.6` (60% effect)
- At `lum = 1.0` (white): `weight = 0.6` (60% effect)

**Example at -2 EV**:
| Luminance | Weight | Current Result | New Result | Improvement |
|-----------|--------|----------------|------------|-------------|
| 0.2 (shadow) | 0.75 | 0.05 | 0.07 | 40% lighter |
| 0.5 (midtone) | 1.0 | 0.125 | 0.125 | Same (full effect) |
| 0.9 (highlight) | 0.7 | 0.225 | 0.28 | 24% lighter |

**Rationale**:
1. **Matches Lightroom research**: Professional tools use midtone-biased exposure for perceptual naturalness
2. **Symmetric behavior**: Same logic for positive and negative exposure
3. **Smooth Gaussian curve**: No discontinuities or artifacts
4. **Context-appropriate**: We're post-processing sRGB (like Snapseed), not processing RAW (like darktable)
5. **Configurable**: Can tune sigma and minWeight if needed

**Implementation Details**:

Files modified:
1. **`src/lib/webgl/ImageGrader.ts`** (lines 131-160)
   - Added Gaussian weight calculation before exposure
   - Applied weighted exposure: `linear *= pow(2.0, exposure * weight)`
   - Kept existing highlight compression for positive exposure

2. **`src/app/api/photos/[name]/export/route.ts`** (lines 44-79, 85-99, 229-239)
   - Updated `filmicExposureCurve()` to include Gaussian weighting
   - Increased curve sampling from 17 to 33 points for smoother Gaussian interpolation
   - Updated comments to reflect midtone-biased approach

**Testing Plan**:
1. Bright image at -2 EV: Verify highlights (0.9 luminance) become ~0.28 instead of 0.225
2. Normal image at -1 EV: Should feel like Lightroom's exposure slider
3. Dark image at +2 EV: Verify positive exposure still works with highlight compression
4. Export matching: Compare full-resolution export to preview

**Parameter Tuning Options**:

Current (balanced):
- `sigma = 0.3`, `minWeight = 0.6`

More aggressive (stronger highlight/shadow protection):
- `sigma = 0.25`, `minWeight = 0.4`
- Trade-off: May feel less like "exposure" and more like "contrast"

Subtle (closer to linear):
- `sigma = 0.4`, `minWeight = 0.8`
- Trade-off: Smaller improvement in highlight preservation

**References**:
- [Darktable exposure.c source code](https://github.com/darktable-org/darktable/blob/master/src/iop/exposure.c)
- [Darktable filmic.c source code](https://github.com/darktable-org/darktable/blob/master/src/iop/filmic.c)
- [Snapseed Tonal Contrast Help](https://support.google.com/snapseed/answer/6158216?hl=en)
- [Lightroom Tone Control](https://helpx.adobe.com/lightroom-classic/help/tone-control-adjustment.html)
